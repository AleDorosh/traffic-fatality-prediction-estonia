...
![Fatality rate by accident type, urban vs rural](figures/eda_accident_type.png)
*Fatality rate by accident type - urban vs rural. Single-vehicle accidents dominate rural fatalities; pedestrian accidents are the deadliest category in urban.*

![Fatality rate by lighting condition, urban vs rural](figures/eda_lighting.png)
*Fatality rate by lighting condition - urban vs rural. Dark conditions without lighting are the highest-risk category in both contexts.*

---

## Results

**Final model:** Logistic Regression with `class_weight='balanced'`. Random Forest was tested and rejected - it assigned near-identical probabilities to fatal and non-fatal cases, a known failure on severely imbalanced data with few positive cases.

| Experiment | Train fatal | Test fatal | ROC-AUC | Recall |
|---|---|---|---|---|
| Urban validation | 51 | 13 | 0.691 | 0.615 |
| Urban to Rural | 64 | 70 | 0.635 | 0.500 |
| Rural to Urban (flipped) | 70 | 64 | 0.733 | 0.641 |
| National baseline | 286 | 71 | 0.780 | 0.282 |

ROC-AUC is the primary comparison metric - it is threshold-independent and measures how well the model ranks fatal accidents above non-fatal ones regardless of where you draw the cutoff. Recall figures use the F1-optimal threshold for each experiment, which means they are not directly comparable across rows. The national baseline recall (0.282) looks low relative to urban to rural (0.500), but the national model has a higher AUC and would catch more fatals at a lower threshold - the F1-optimal point just sits differently when the dataset is larger.

**What the numbers say:**

The urban to rural AUC drop of 0.056 confirms distribution shift is real - the model loses discrimination when applied to an unseen context. The more telling result is the national baseline: same model, same features, 6x more fatal training cases, AUC jumps to 0.780. The features work. The urban experiment was constrained by having only 64 fatal training cases, not by weak features.
...
One practical finding: the decision threshold calibrated on urban validation (0.797) does not transfer to rural. Applying it to rural drops recall from 0.500 to 0.214. Distribution shift compresses the probability scale, not just the ranking.

![ROC curves - urban validation and rural test](figures/roc_curves.png)
*ROC curves for urban validation and rural test set. The gap between the two curves is the cost of distribution shift.*
