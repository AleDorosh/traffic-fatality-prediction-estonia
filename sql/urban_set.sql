...
DELETE FROM urban_set
WHERE accident_id IN (
    SELECT accident_id FROM urban_set
    GROUP BY accident_id HAVING COUNT(*) > 1
);
SELECT 'After dedup: ' || COUNT(*) FROM urban_set;

-- Step 2: Remove rows missing speed_limit (non-negotiable feature)
DELETE FROM urban_set 
WHERE speed_limit IS NULL;
SELECT 'After speed_limit NULL drop: ' || COUNT(*) FROM urban_set;
...
SELECT 'After > 1 NULL drop: ' || COUNT(*) FROM urban_set;
...
-- Step 5. Replace Pole teada, Teadmata etc values in secondary columns with UNKNOWN
UPDATE urban_set
